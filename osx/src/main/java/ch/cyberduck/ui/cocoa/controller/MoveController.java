    ...
    public void rename(final Map<Path, Path> selected) {
        final DefaultMainAction action = new DefaultMainAction() {
            ...
            public void run() {
                ...
                final MoveWorker move = new MoveWorker(selected, pool.getHost().getProtocol().getStatefulness() == Protocol.Statefulness.stateful ? SessionPoolFactory.create(parent, pool.getHost(), cache) : pool, cache, parent, LoginCallbackFactory.get(parent)) {
                    @Override
